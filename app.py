...
# A mismatch usually means the processed CSV was generated by an older
# pipeline run. Refreshing from FRED fetches clean data via the API.
missing_cols = [k for k in FRED_SERIES if k not in df_raw.columns]
if missing_cols:
    st.warning(
        "⚠️ Cached data appears outdated. "
        "Click **Refresh from FRED** in the sidebar to reload."
    )
    st.stop()
